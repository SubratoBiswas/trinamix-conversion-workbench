"""Fuzzy duplicate / entity resolution — the pure clustering policy (Phase 4, slice 4).

The multi-source merge (``merge_dedupe``) collapses records on an EXACT business key. This
finds the ones it can't: the same supplier/customer/item appearing across source systems as
"Acme Inc", "Acme, Inc." and "ACME INCORPORATED". It blocks candidates (so it stays
near-linear, not O(n squared)), scores pairs on the object's identity fields with a fuzzy
similarity, clusters them with union-find, and returns each suspected-duplicate cluster with
a confidence score and the field-level evidence.

Pure — pandas + stdlib ``difflib``/``re``, no DB, network or model — so the matching logic is
unit-testable in isolation. The OPTIONAL AI adjudication pass that confirms/denies borderline
clusters via the configured LLM is I/O and stays in ``app.services.entity_resolution``.
"""
