...
_CUSTOMER_08AUG = _DATA / "customer_mapping_08aug.json"


async def seed_customer_mapping_08aug() -> dict:
    """The 08-Aug Customer corrections: sheet-scoped blanks (UDCP, DFF segments on
    ...
    source system, and the PARTIES Party Usage Code conditional. Same one-store,
    per-row-resilient, sheet-scoped path as the other customer seeds; dated 08-Aug
    so it wins over the 03/06/07-Aug documents while still losing to a later human
    edit. Count mismatches and computed OSR remaps are tracked in the file's
    ``_open_items`` and handled separately — they are not field mappings."""
    import json as _json
    if not _CUSTOMER_08AUG.exists():
        return {"seeded": 0, "note": "customer_mapping_08aug.json not found"}
    try:
        doc = _json.loads(_CUSTOMER_08AUG.read_text(encoding="utf-8"))
    except Exception as exc:  # noqa: BLE001
        return {"seeded": 0, "error": str(exc)}

    nid = await _nextpower_client_id()
    eff = _effective_date_of(doc)
    label = doc.get("_label") or "NXT Customer mapping (08-Aug-2026)"
    erp = "netsuite"
    _ACTIONS = {
        "derive": mapping_store.SOURCE_COLUMN,
        "constant": mapping_store.DEFAULT_VALUE,
    ...
    for r in doc.get("rules") or []:
        ...
        try:
            ...
            errors.append({"target_field": tgt, "error": f"{type(exc).__name__}: {exc}"})
            logger.exception("customer 08-Aug seed: %r failed to record", tgt)
            continue
        if row is None:
            retired += 1
            continue
    ...
    out = {**counts, "retired": retired, "skipped": skipped,
           "errors": errors, "error_count": len(errors),
           "effective_date": doc.get("_effective_date")}
    logger.info("customer 08-Aug mapping seed: %s", out)
    return out
