...
set "DEPLOY_NOTE=Bundle download - the zip is built on DISK and streamed, not held in memory twice. Peak memory was 2x the bundle on a 512MB instance and nothing was sent until the last entry compressed, so a slow zip and a hung one looked identical from the browser - which is what makes a stuck Packaging invite a second click and a second concurrent build. download-all also no longer REGENERATES inline: a missing file is now an instant 409 naming the interface instead of six silent rebuilds inside one request. Verified end-to-end against an in-memory Mongo - FileResponse, correct entries, and a deleted file refused by name. Suite 1242 passing, 14 skipped, 0 failing."
