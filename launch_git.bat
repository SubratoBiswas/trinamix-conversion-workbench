...
set "DEPLOY_NOTE=THE MAPPINGS IN THE UI NOW REACH THE OUTPUT. Measured live on NextPower Supplier Test / Supplier Site - all SEVEN strategy constants were overriding the value on screen and three of them differed: Receipt Routing showed 3 and shipped DIRECT, Invoice Match Option showed R and shipped Receipt, Match Approval Level showed 3 and shipped 3-Way. Every one of those rows was approved, carried a fixed value and was dated 03 or 04-Aug. The directive that beat them is dated 13-JUL - three weeks older - and it won only because the newer statement was stamped learning-engine. That made AUTHORSHIP decisive when START_HERE says newest wins and authorship is provenance only. The precedence is now the date, as the architecture says. The seeded-row case the old rule protected is a date question too and is covered by its own test. Confirmed by Subrato. Affects supplier, customer, BOM, Item and Employee. Suite 1271 passing, 13 skipped, 0 failing. REGENERATE everything after this."
