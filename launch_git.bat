...
set "DEPLOY_NOTE=Phase 4 slice 4: split entity resolution into a pure domain policy plus an LLM adapter. Behaviour-preserving and verified byte-identical - no output change is possible from this slice. entity_resolution was mostly pure fuzzy duplicate clustering (identity-field detection, pair scoring, union-find, string similarity) but carried one impure function, ai_adjudicate_clusters, an async best-effort LLM call. So this is a store/policy SPLIT: the deterministic clustering policy moved to app/domain/entity/resolution.py (pure), while ai_adjudicate_clusters stays in services/entity_resolution.py and re-exports the policy, so find_duplicate_clusters / detect_identity_fields resolve unchanged for routers/operations and decision_service. Verified: all 16 pure names present on the shim by identity, the adapter is byte-identical in services and absent from the domain, and a differential ran the DEPLOYED baseline against the split across the matching functions over duplicate-variant frames - 50 comparisons - ALL identical - and 7 new unit tests pass. entity_resolution is a duplicate-detection tool, not the generation path, so the customer regen confirms no collateral breakage. After deploy REGENERATE the customer project with Generate all and download and confirm it still matches the golden byte-for-byte."
