...
set "DEPLOY_NOTE=Phase 4 slice 1: relocate the customer merge into the domain. Behaviour-preserving and verified byte-identical - no output change is possible from this slice. customer_merge was about 1,460 lines of pure DataFrame-only logic (grain classification, entity enrichment, party linkage, owned-field stamping, DFF/UDCP, contact fan-out, party numbering, date coercion) with no I/O or models, so it moved VERBATIM to app/domain/customer/merge.py; services/customer_merge.py is now a 21-line re-export shim that copies every module-level name by reference. That keeps every _cm.X caller unchanged and, crucially, keeps _OWNED_OVERRIDE as ONE ContextVar so the per-sheet owned-field context still works across the seam. Verified: all 77 baseline names present on the shim by identity, and a differential ran the DEPLOYED baseline against the shim across every public function including the orchestrators with the ContextVar set both ways - 110 comparisons - ALL identical - and 8 new unit tests pass. After deploy REGENERATE the customer project with Generate all and download and confirm the output still matches the golden byte-for-byte."
