...
set "DEPLOY_NOTE=Phase 4 slice 3: relocate the value cleansing rules into the domain. Behaviour-preserving and verified byte-identical - no output change is possible from this slice. cleansing_rules was about 393 lines of pure DataFrame-only logic (whitespace/punct normalisation, special-character folding, case/title-casing, legal-suffix canonicalisation, per-column family profile) with no I/O, no models and no app imports, so it moved VERBATIM to app/domain/cleansing.py; services/cleansing_rules.py is now a 16-line re-export shim so routers/operations (cr.X) and generate_dq (cleanse_frame, _norm_protect) keep working unchanged. Verified: all 45 baseline names present on the shim by identity, and a differential ran the DEPLOYED baseline against the shim across the family appliers, is_text_column, default_profile and cleanse_frame over an 18-string battery and representative frames - 144 comparisons - ALL identical - and 7 new unit tests pass. cleanse_frame runs in the generate-time DQ step, so the customer path exercises this. After deploy REGENERATE the customer project with Generate all and download and confirm it still matches the golden byte-for-byte."
