...
set "DEPLOY_NOTE=REC-06 is now a user-driven Duplicate Suspects review, NOT an auto-delete. The tool only detects shipping addresses that exactly match a billing address for the same customer and lists them for the analyst; nothing is removed until the analyst chooses Delete or Merge and regenerates. Delete drops the shipping duplicate across all site sheets. Merge folds it onto the billing site so one site carries both BILL_TO and SHIP_TO. Decisions are saved on the project and applied at the next Generate all and download. A new Duplicate suspects button sits beside Generate all and download and opens the review. REC-02 fax maps to its own PHONE contact point with Phone Line Type FAX, dormant until a fax column exists. After deploy REGENERATE the customer project with Generate all and download, click Duplicate suspects, choose delete or merge on a few, Save, then Generate all again and confirm the output reflects only your choices. ALSO in this deploy: Employee HDL AssignmentNumber now prefixes by worker type, C plus number for a contingent worker and E plus number for an employee, using the id digits, so a contingent id like C-100003 becomes C100003 instead of EC-100003, and the manager assignment number uses the matching prefix so supervisor links resolve. Regenerate the Employee HDL and confirm the Worker AssignmentSupervisor tab shows C or E prefixes with no EC values."
