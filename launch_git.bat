...
set "DEPLOY_NOTE=THE REAL ONE, measured live. Generation was SUCCEEDING and the poller was calling it failed. One Supplier Site interface - 5,476 rows, 88 columns, into a 210-column template - finished in about 570s on the free instance and wrote SupplierSite_Template.xlsx, 5,405 rows, 646KB. The staleness guard hit its hard-coded 600 seconds a few seconds later and flipped it to failed. The file was on disk and downloadable the whole time - I downloaded it. Two causes, both fixed: the guard now asks the DISK before believing the clock, and reports ready when an artifact from this run exists; and generate-merged-all stamped the start time on all six carriers at kick-off while only two run at a time, so the last pair had been generating for twenty minutes before their turn - each carrier is now re-stamped when its work actually begins. Ceiling raised to 2400s and now settable via GENERATION_TIMEOUT_SECONDS. Suite 1261 passing, 13 skipped, 0 failing."
