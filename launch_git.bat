...
set "DEPLOY_NOTE=Phase 3 slice 2: split the FBDI layout module into a store plus a pure decision module. Behaviour-preserving and verified byte-identical - no output change is possible from this slice. The column reorder, the END terminator, the Customer load scope and the Oracle zip/CSV file naming moved to app/domain/fbdi/layout.py as pure functions that take the loaded spec as an argument; services/supplier_fbdi_layout.py keeps reading the four JSON specs and now delegates to that policy, with every public signature unchanged. A differential harness compared the DEPLOYED baseline against the new store plus policy across all 15 Customer, 5 Supplier and 4 BOM interfaces - 1,362 frame cases including the 87-wide LOCATIONS duplicate-name sheet, plus every name and scope lookup - ALL identical - and 20 new unit tests pass. After deploy REGENERATE the customer project with Generate all and download and confirm the output still matches the golden byte-for-byte."
