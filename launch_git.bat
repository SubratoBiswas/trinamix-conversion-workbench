...
set "DEPLOY_NOTE=Phase 3 slice 1: split strategy_overlay into a store plus a pure directive policy. Behaviour-preserving and verified byte-identical - no output change is possible from this slice. The decision of which directive applies to an object, field and source moved to app/domain/directives/policy.py as pure functions that take the loaded caches as arguments; strategy_overlay keeps reading the 5 JSON files and now delegates to that policy, with every public signature unchanged. A differential harness compared the DEPLOYED strategy_overlay baseline against the new policy across 5,481 directive cases plus blank_fields, referenced_columns, rule configs and the frame pass - ALL identical - and 15 new unit tests pass. After deploy REGENERATE the customer project with Generate all and download and confirm the output still matches the golden byte-for-byte."
