...
set "DEPLOY_NOTE=Generation now RETURNS when the file is done. Measured live today - a Supplier Site generate wrote its artifact at 150s and the API went on saying generating past 410s. The file was complete and downloadable for that whole four minutes; I downloaded it while the poller still said in progress. All of it was capture_learnings_from_conversion, awaited AFTER the artifact was written and recorded - one Mongo upsert per mapped field, in sequence, contributing nothing to the file. The existing skip already conceded the point for objects over 300 fields; Supplier Site has 211 so it paid in full. It is now scheduled instead of awaited, so the deliverable stops waiting on a side effect. Expect roughly 150s instead of 410s for Supplier Site. Suite 1267 passing, 13 skipped, 0 failing."
