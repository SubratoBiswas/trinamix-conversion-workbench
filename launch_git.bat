...
set "DEPLOY_NOTE=Phase 4 slice 2: relocate the BOM structure reshape into the domain. Behaviour-preserving and verified byte-identical - no output change is possible from this slice. bom_structure_service was about 274 lines of pure DataFrame-only logic (BOM interface grain reshape, Item Sequence renumbering, mandatory-field diagnostics, batch stamping) with no I/O, no models and no app imports, so it moved VERBATIM to app/domain/bom/structure.py; services/bom_structure_service.py is now a 16-line re-export shim so the two lazy reshape_for_sheet imports in output_service keep resolving to the same object. Verified: all 16 baseline names present on the shim by identity, and a differential ran the DEPLOYED baseline against the shim across reshape_for_sheet and missing_mandatory over the four BOM sheets plus empty/partial frames and the constants - 59 comparisons - ALL identical - and 5 new unit tests pass. The customer golden does not exercise the BOM path, so its regen confirms no collateral breakage; a BOM project regen exercises the reshape itself. After deploy REGENERATE the customer project with Generate all and download and confirm it still matches the golden byte-for-byte."
