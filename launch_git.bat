...
set "DEPLOY_NOTE=ROOT CAUSE of the supplier FBDI template download. 3_SupplierSite_POZ_SUPPLIER_SITES_INT.xlsm carries seven out-of-spec font family values, and they are in xl/comments1.xml - NOT in xl/styles.xml, which is the only part xlsx_repair cleaned. So the repair reported nothing out of range, changed nothing, and openpyxl refused the workbook write-capable: the filled Supplier Site template could never be produced. The parser never saw it because read_only skips comments. repair_bytes now covers styles, sharedStrings and comments. All nine bundled templates open write-capable; the six supplier interfaces generate and bundle end-to-end. A test that had been SKIPPING over this exact error now runs - skips 14 to 13. Suite 1250 passing, 13 skipped, 0 failing. SEPARATE ISSUE, not fixed by this: on the free Render instance all six timed out at 600s. Peak memory measured at 1.1 GB against 512 MB available. Set MERGE_CONCURRENCY=1 in the dashboard or size the instance up."
