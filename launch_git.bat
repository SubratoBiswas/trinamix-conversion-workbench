...
set "DEPLOY_NOTE=Phase 3 slice 5: unify the write-time precedence contests behind one ordering. Behaviour-preserving and verified byte-identical - no output change is possible from this slice. The four whichever-is-latest contests (conversion_rule_wins, person_is_newer, wide_directive_wins and the overlay _explicit decision) now all delegate to one pick_latest over a common Statement, so the rule is written once and the tie-breaks that used to be scattered inline date comparisons are explicit named terms (conversion beats directive on a tie, exact beats wide, and an undated authored rule is treated as newest). Scope is the two write-time contests only; the library store keeps its own ordering by design; the call sites are unchanged. A differential ran the DEPLOYED baseline against the unified policy across an exhaustive battery including exact duplicate instants and undated values - 27,105 comparisons - ALL identical - and 13 new unit tests pass. After deploy REGENERATE the customer project with Generate all and download and confirm the output still matches the golden byte-for-byte."
