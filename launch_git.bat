...
set "DEPLOY_NOTE=Phase 3 slice 4: relocate the dated mapping-store read/resolve core into the domain. Behaviour-preserving and verified byte-identical - no output change is possible from this slice. The pure whichever-is-latest rule (the decision vocabulary, the Entry, how a row or a mapping becomes an Entry, which entries apply, and which one wins) moved to app/domain/store/resolver.py; services/mapping_store.py keeps only the Mongo adapter and re-exports the core, so every mapping_store.X reference is unchanged (mapping_store.Entry and mapping_store.resolve are the domain objects). A differential fed the SAME rows and mappings to the DEPLOYED baseline and the new core - 5,760 rows, 5,265 resolve cases and 1,944 mapping objects across every winner, ordering tie-break and predicate - ALL identical - and 17 new unit tests pass. After deploy REGENERATE the customer project with Generate all and download and confirm the output still matches the golden byte-for-byte."
