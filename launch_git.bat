...
set "DEPLOY_NOTE=Phase 3 slice 3: extract the mapping-vs-directive precedence decision from _transform_frame into the domain. Behaviour-preserving and verified byte-identical - no output change is possible from this slice. The _explicit gate (does an analysts own approved value or authored rule outrank the strategy directive, by whichever is latest) moved into app/domain/precedence/policy.py as a pure, unit-tested function; output_service now passes primitives and consumes the boolean, dropping 98 lines. A differential compared the OLD inline expression against the new function across the full product of status, approver, dates, source column, fixed value and rule dates - 94,080 cases - ALL identical - and 12 new unit tests pass. After deploy REGENERATE the customer project with Generate all and download and confirm the output still matches the golden byte-for-byte."
